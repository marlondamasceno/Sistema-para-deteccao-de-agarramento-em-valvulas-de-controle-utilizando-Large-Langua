...
# Rev.09: Acrescida da solicitação de apresentar os valores de TrPV e TrMV
# Rev.10: Inserido a apresentação da quantidade de tokens na IDE Python e salvar em arquivo excel
#
...
# ESTE TRECHO DESTINA-SE A UTILIZAR QUANDO HÁ NECESSIDADE DE CONSIDERAR-SE A VARIÁVEL "T_limite" (DEIXAR COMENTADO CASO NÃO FOR USAR)
##PROMPT_FIXO +="""
##
...
##    Considerar T_limite = 12 segundos. Informar o TrPV e TrMV de cada distúrbio. Lembre-se das TÉCNICAS PARA AVALIAÇÃO DE AGARRAMENTO EM VÁLVULAS DE CONTROLE.
##    
...
# ESTE TRECHO DESTINA-SE A UTILIZAR QUANDO NÃO HÁ NECESSIDADE DE CONSIDERAR-SE A VARIÁVEL "T_limite", SENDO O AGARRAMENTO AVALIADO APENAS PELO TEMPO DE AMOSTRAGEM (DEIXAR COMENTADO CASO NÃO FOR USAR)
PROMPT_FIXO +="""
...
Lembre-se das TÉCNICAS PARA AVALIAÇÃO DE AGARRAMENTO EM VÁLVULAS DE CONTROLE.
    ...
    a. **Verificar agarramento nos distúrbios classificados como transientes:** Verifique se há indícios de agarramento nos trechos de distúrbio classificados como transientes.
    Informar o TrPV e TrMV de cada distúrbio. Lembre-se das TÉCNICAS PARA AVALIAÇÃO DE AGARRAMENTO EM VÁLVULAS DE CONTROLE.
...
# TRECHO FINAL DO PROMPT, SOLICITAÇÃO DA GERAÇÃO DE RELATÓRIO
PROMPT_FIXO +="""
...
def gerar_pdf_local(pdf_path, tag, data_str, imagem_path, paragrafo):
    ...
    # Imagem (TESTE VALIDAÇÃO IMAGEM)
    if imagem_path is not None and os.path.exists(imagem_path): #ADICIONADO
        ...
        print("AVISO: Nenhuma imagem foi gerada pela LLM.") #ADICIONADO
...
def run_analysis(uploaded_file, ciclos_analises):
    ...
    # Arquivo Excel - tabela de tokens e custos
    arquivo_custos = r"C:\Users\Windows 10\Desktop\Mestrado UFOP - Márlon\01 - Projetos LLM\resultados\Custos_Testagens.xlsx"
    ...
    for i in range(1, ciclos_analises + 1):
        ...
        # Exibir quantidade de tokens utilizados
        if resp.usage:
            ...
            # Custo de tokens do GPT-5.2 em US$ por 1 milhão de tokens
            preco_entrada_usd = 1.75
            preco_saida_usd = 14.00

            # Cotação do dólar (base em 08.2026)
            dolar_real = 5.50

            # Custo em dólares (US$)
            custo_entrada_usd = (tokens_entrada / 1_000_000) * preco_entrada_usd
            custo_saida_usd = (tokens_saida / 1_000_000) * preco_saida_usd
            custo_total_usd = custo_entrada_usd + custo_saida_usd

            # Custo em reais (BRL)
            custo_entrada_brl = custo_entrada_usd * dolar_real
            custo_saida_brl = custo_saida_usd * dolar_real
            custo_total_brl = custo_total_usd * dolar_real

            # Exibição dos resultados (na IDE Python)
            print("\n==========================================")
            print(f"TOKENS E CUSTO - CICLO: {i}")
            print("==========================================")
            ...
            print(f"Custo tokens de entrada : R$ {custo_entrada_brl:.4f}")
            print(f"Custo tokens de saída   : R$ {custo_saida_brl:.4f}")
            print(f"Custo total da operação : R$ {custo_total_brl:.4f}")

            print("==========================================\n")

            # Registrar os custos da testagem no arquivo Excel (mesma pasta em que imprime os relatórios)
            nova_testagem = pd.DataFrame([{
                ...
                "Tokens de Saída": tokens_saida,
                "Total de Tokens": tokens_total,
                "Cotação Dólar (R$)": dolar_real,
                "Custo Entrada (US$)": custo_entrada_usd,
                "Custo Saída (US$)": custo_saida_usd,
                "Custo Total (US$)": custo_total_usd,
                "Custo Entrada (R$)": custo_entrada_brl,
                "Custo Saída (R$)": custo_saida_brl,
                "Custo Total (R$)": custo_total_brl,
                "Análise Diagnóstico": [],
            }])
            ...
            if os.path.exists(arquivo_custos):
                ...
                historico = pd.read_excel(arquivo_custos)

                # Acrescenta a nova testagem
                historico = pd.concat(
                    [historico, nova_testagem],
                    ignore_index=True
                )

            else:

                # Primeira testagem: cria o histórico
                historico = nova_testagem

            # Salva/atualiza o arquivo Excel
            historico.to_excel(
                arquivo_custos,
                index=False
            )

            print(f"Custos registrados em: {arquivo_custos}")
