...
1. O comportamento deve ser aproximadamente periódico, com repetição clara de ciclos. Não há momentos de estabilidade em um trecho com oscilação.
2. A amplitude das oscilações deve ser aproximadamente constante ao longo do tempo e não é amortecida.
...
5. Deve existir coerência temporal entre MV e PV compatível com comportamento não linear, como por exemplo rampas em MV e saltos na PV.
...
Passo 2 – Avaliar se estas oscilações identificadas na PV e na MV obedecem aos padrões típicos de forma de onda para agarramento, conforme
os Critérios obrigatórios para classificar um trecho como OSCILAÇÃO (limit cycle).
...
Passo 4 – Gerar uma imagem apenas para o gráfico do Resultado da análise da amostra conforme formatação solicitada. Publicar/anexar a imagem gerada do gráfico como output e na formatação solicitada. 
...
Passo 6 – Verificar se o Relatório Técnico de análise foi publicado/anexado conforme solicitado.
