...
entre a reação do sinal de MV e a reação do sinal da PV quando ocorre uma alteração no valor do SP, ou seja, quando ocorre um distúrbio do tipo transiente, ocorre mudança no Set Point (SP) e é gerado um
valor de erro significativo no sistema maior que o critério C determinado, e ocorre também um grande atraso no tempo de reação da PV em relação ao tempo de reação da MV, então esta situação é classificada como
agarramento em um transiente.
