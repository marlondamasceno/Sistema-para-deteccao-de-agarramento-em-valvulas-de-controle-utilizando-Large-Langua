...
Seu objetivo final é identificar se há ocorrência de distúrbios tipo oscilações por meio dos sinais de PV e MV fornecidos por um arquivo em extensão .csv de
amostra coletado de uma malha de controle e gerar um relatório técnico final. No relatório final,
os trechos onde ocorrem as oscilações detectadas devem ser destacados.
...
Para concluir que ocorreu uma oscilação em um trecho, o Aspecto 5, o Aspecto 6, o Aspecto 7, o Aspecto 8 e o Aspecto 9 devem ser atendidos simultaneamente.
...
- A Legenda da descrição da Figura deve ser “Gráfico de Amostra de Sinal – Avaliação de distúrbios em Malha de Controle”.
...
- O fundo dos trechos onde há oscilação DEVE ser preenchido com a cor Amarela, código hexadecimal #F1C40F, para destacar e permitir
melhor visualização e entendimento do gráfico pelo usuário.
- Devem ser inseridos marcadores para delimitar o início do(s) distúrbio(s) e o final do(s) distúrbio(s), que deverão ser representados por linhas verticais tracejadas na cor
Cinza, código hexadecimal #2C2C2C, com marcador/label de “Início” para o delimitador do início da oscilação e marcador/label de “Término” para o delimitador
de final da oscilação para permitir melhor visualização e entendimento do gráfico pelo usuário.
- O primeiro instante de cada zero-crossings identificado deve ser circulados com a cor Vermelha, código hexadecimal #E74C3C e com espessura de linha 2px.
- A Legenda do Gráfico deve ser apresentada com a identificação dos sinais de SP, MV, PV e Trecho com distúrbio(s) (caso haja algum trecho com distúrbio). 
...
- O Texto técnico do diagnóstico da Avaliação deve informar os instantes iniciais e finais de cada trecho de distúrbio detectado na malha de controle.
- Ao final do Texto técnico do diagnóstico da Avaliação deve estar a conclusão final informando se foi(foram) constatado(constatados) distúrbio(s) no sistema de controle ou
se não foi(foram) constatado(constatados) distúrbio(s) no sistema de controle.
...
2 - **Classificar os trechos com distúrbio:** Classifique quais trechos identificados como distúrbio são transientes, quais trechos distúrbios isolados e quais trechos oscilações.
...
3 - **Avaliar se os trechos classificados como oscilações são agarramento:** Verifique se os trechos com distúrbio que foram classificados como oscilações possuem as características de terem sido causados por
agarramento (sticion) na válvula de controle. Lembre-se das TÉCNICAS PARA AVALIAÇÃO DE AGARRAMENTO EM VÁLVULAS DE CONTROLE

4 - **Destacar os trechos classificados como oscilações causadas por agarramento:** Destaque os trechos com distúrbio que foram classificados como oscilações causadas por agarramento. Cada trecho de distúrbio possui
um instante inicial ti e um instante final tf.

5 – **Gerar o Relatório Técnico:** Gere o Relatório Técnico da análise conforme modelo solicitado e apresente o(s) agarramento(s) detectado(s) na amostra em destaque.
